class World {
  ...
  checkPoisenThrow() {
    ...
    if (this.keyboard.D && currentTime - this.lastPoisenThrowTime > this.poisenThrowCooldown) {
      if (this.player.poisen > 0) {
        let bottleDirection = this.player.otherDirection ? 'left' : 'right';
        let bottle = new ThrowPoisen(this.player.x, this.player.y, bottleDirection);
        if (!isMutetd) {
  ...
  checkBubbleThrow() {
    const currentTime = Date.now();
    if (this.keyboard.SPACE && currentTime - this.lastShotTime > this.shotCooldown) {
      let bubbleDirection = this.player.otherDirection ? 'left' : 'right';
      let bubble = new ThrowBubble(this.player.x, this.player.y, bubbleDirection);
      if (!isMutetd) {
